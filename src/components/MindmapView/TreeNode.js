...
export default class TreeNode {
  ...
  draw(currentBranchColor) {
    ...
    this.labelWidth = Math.ceil(
      Math.max(...this.labelLines.map((c) => this.ctx.measureText(c).width))
    );
    ...
    if (this.isLeaf) {
      ...
      if (this.isRoot) {
        var branchColors = this.children.map((c) =>
          utils.generateRandomColor(treeProperties.useGrayscale)
        );
      ...
      this.canvas.width =
        leftMargin + Math.max(...canvases.map((c) => c.width));
      this.ctx.font = treeProperties.fontSize.val + "px " + fontFamily;
